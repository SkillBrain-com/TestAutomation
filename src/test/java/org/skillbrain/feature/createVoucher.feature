
@cris
Feature: Testing voucher page

  Scenario: Create attraction without filling in mandatory fields
    Given User navigates to login page
    When User logs in with valid credentials
    Then Check user is logged in
    When I click on attractions sidebar
    And I click on Vouchers
    And I click on Create voucher button
    And I input voucher's name of "ADRIAN"
    And I input voucher's discount of 22
    And I input a "code" manually
    When I click on Save button
    Then I check if the "VOUCHERTEST22142" appears in list
    Then Delete the voucher
    And I click on Create voucher button
    And I input voucher's name of "Adrian"
    And I input voucher's discount of 22
    #And I input voucher's name of "Adrian"
    #And I input voucher's discount of 22
    And I input voucher's discount of 22
    Then I Open Generator
    And I input number of codes
    And I input code lenght
    Then I Generate codes
    When I click on Save button
    Then I check if the "VOUCHERTEST22142" appears in list
    Then Delete the voucher


  Scenario: Create a voucher for a specific event only and check if the works
    Given User navigates to login page
    When User logs in with valid credentials
    Then Check user is logged in
    When I click on attractions sidebar
    And I click on my attractions
    And  Click the Create attraction button
    Then Enter the name of the event with "Eveniment 1"
    Then I expand the Location accordion
    And Enter the name of location address with "Iasi"
    Then I click on Next button
    And I click the Create ticket button
    Then Enter the name of ticket with "Eveniment"
    And I choose the price field with 50
    Then I click the Save button
    And I click on Preview & Publish tab button
    Then I click the Publish Attraction button
    When I click on attractions sidebar
    And I click on Vouchers

    When I click on Vouchers
    And I click on Create voucher button
    Then I Enter the name of the voucher
    And I input voucher discount
    Then Select the attraction for which the voucher applies
    And Add a code
    Then I click on Save button
    Then I check if the "Testare1" appears in list
    When I click on my attractions
    And I click on GoToHub button
    Then I click on the attraction
    And I click on the Buy Now button from the eveniment
    Then Enter the code
    And Verify if the code works


  Scenario: Testing a voucher for one attraction to see if it works for another attraction
    Given User navigates to login page
    When User logs in with valid credentials
    Then Check user is logged in
    When I click on attractions sidebar
    And I click on my attractions
    And I click on GoToHub button
    Then I click one attraction
    And I click on the Buy Now button
    Then Enter the code
    And Check if the error with invalid code appears

  Scenario: Create a voucher with special characters
    Given User navigates to login page
    When User logs in with valid credentials
    Then Check user is logged in
    When I click on attractions sidebar
    And I click on Vouchers
    And I click on Create voucher button
    Then I Enter the name of the voucher
    And I input voucher discount
    And Add a code with special characters
    And  Check if the error appears
    Then I check if the "Testare4" appears in list
    #And I click on my attractions
    #Then I click on GoToHub button

  @maria
  Scenario: Verify if the final price with a voucher with a discount greater than the ticket value is free
    Given User navigates to login page
    When User logs in with valid credentials
    Then Check user is logged in
    When I click on attractions sidebar
    And I click on Vouchers
    And I click on Create voucher button
    Then I Enter the name of the voucher
    And I input voucher discount
    And I choose the RON currency
    And I click on the Generate codes
    And I enter the number of codes to "1"
    And I enter the Generate button
    Then I enter Save button
    When I click on my attractions
    And I click on GoToHub button
    Then I click on the attractionTestPrice
    And I click on the Buy Now button from the attraction
    Then Put the generated code
    And Verify if the price is free

